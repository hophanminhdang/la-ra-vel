...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Mar 28, 2025 at 02:55 PM
...
-- Database: `solution_a`
--
CREATE DATABASE IF NOT EXISTS `solution_a` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `solution_a`;
...
---lấy danh sách 7 người theo thứ tự tên A-Z
SELECT user_id, user_name, user_email  
FROM users  
ORDER BY user_name ASC  
LIMIT 7;
...
---Lấy ra danh sách người dùng trong đó email người dùng là Gmail và tên người dùng bắt đầu bằng chữ m
SELECT * FROM users 
WHERE user_email LIKE '%@gmail.com' 
AND user_name LIKE 'm%';
---Lấy danh sách người dùng có email là Gmail, tên chứa chữ "i" và tên dài hơn 5 ký tự
SELECT * FROM users 
WHERE user_email LIKE '%@gmail.com' 
AND user_name LIKE '%i%' 
AND LENGTH(user_name) > 5;
--- Lấy danh sách người dùng có tên chứa chữ "a",chiều dài từ 5 đến 9 ký tự, email Gmail và trong tên email có chữ "i"
SELECT * FROM users 
WHERE user_name LIKE '%a%' 
...
WHERE 
    ...
    OR 
    (user_email LIKE '%@gmail.com' AND SUBSTRING_INDEX(user_email, '@', 1) LIKE '%i%');
